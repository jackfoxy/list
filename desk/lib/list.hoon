...
::  +averageBy: projection list
++  average-by
::

::
::  Applies a function to each element in a list and then returns a list of
::  values v where the applied function returned Some(v). Returns an empty list
::  when the input list is empty or when the applied chooser function returns.
...
::
::  Divides the input list into lists (chunks) with at a positive number of at
...
::  +removeManyAt: index count source
++  remove-many-at
::
